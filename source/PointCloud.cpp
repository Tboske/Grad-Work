...
PointCloud::PointCloud(const std::string& meshName, const std::vector<float>& pointCloud,
                       const std::vector<uint32_t>& shape, const FPoint3& pos)
	: BaseObject(meshName, pos, L"Resources/MarchingCubesShader.fx")
	  , m_pPointCloudEffect{new Effect(L"Resources/PointShader.fx")} // MarchingCubesShader
...
PointCloud::PointCloud(const std::string& meshName, const std::vector<float>& pointCloud,
                       const std::vector<uint32_t>& shape, const FMatrix4& transform)
...
void PointCloud::Render(ID3D11DeviceContext* pDeviceContext) const
{
	...
	// Set vertex buffer
	constexpr UINT stride = sizeof(CubeInfo);
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		pDeviceContext->Draw(static_cast<UINT>(m_RenderPoints.size()), 0);
	}
...
HRESULT PointCloud::Initialize()
{
	...
	m_pEffect->GetEffect()->GetVariableByName("gTriangulationLUT")->AsShaderResource()->SetResource(
		m_pTriangulationLUT->GetSRV());
...
void PointCloud::InitPointCloud(const std::vector<float>& pointCloud, const std::vector<uint32_t>& shape)
{
	...
	Progress::Start("Initializing"
	                ...
	                , static_cast<float>(GetPos(shape[0], shape[1], shape[2], shape[3]))
	);
	...
	for (uint32_t t = 0; t < 1; ++t) // for now we just use 1 time frame
	{
		...
		for (uint32_t z = 0; z < shape[1] - 1; ++z)
		{
			for (uint32_t y = 0; y < shape[2] - 1; ++y)
			{
				...
				{
					CubeInfo ci;
					ci.pos = FPoint3{static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)};
					ci.cubeID = static_cast<UINT>(GetCubeFillID(z, y, x));
					...
					Progress::SetProgress(static_cast<float>(GetPos(t, z, y, x)));
				}
			}
		}
	}
...
HRESULT PointCloud::CreateVertexBuffer()
{
	...
	bd.ByteWidth = sizeof(CubeInfo) * static_cast<uint32_t>(m_RenderPoints.size());
...
void PointCloud::StartExport() const
{
	...
	Progress::Start("Marching Cubes", "Running Algorithm", static_cast<float>(m_Shape[1] * m_Shape[2] * m_Shape[3]));
	FPoint3 pos;
		...
		for (uint32_t y = 0; y < m_Shape[2] - 1; ++y)
		{
			...
			for (uint32_t x = 0; x < m_Shape[3] - 1; ++x)
			{
				...
				for (int vertIdx = 0; vertexIndices[vertIdx] != -1; vertIdx += 3)
				{
					...
					Progress::SetProgress(static_cast<float>((z * m_Shape[2] * m_Shape[3]) + (y * m_Shape[3]) + x));
				}
			}
		}
	}
	...
	// here we create a new mesh with the generated mesh (Marching Cubes) and directly export it as an OBJ
...
void PointCloud::SlicePointCloud(const FVector3& orientation)
{
	for (CubeInfo& cube : m_RenderPoints)
	{
		...
		if (Dot(vec1, orientation) < 0)
			cube.pos.x += static_cast<float>(*std::ranges::max_element(m_Shape)) * 1.5f;
	}
